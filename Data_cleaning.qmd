...
12 and 13) `r colnames(final_df[,12])` and `r colnames(final_df[,13])` refers to the number of people and households sharing the same postcode. Even if those variables contains a lot of NAs (`r count(is.na(final_df$population` NAs), when we sum across all postcodes we obtain a total population of `r sum(population_df$population`, which is roughly equal to the total population of England and Wales in 2011.
...
15) `r colnames(final_df[,15])`

16) `r colnames(final_df[,16])` is a 95% confidence interval for the income variable. It represents a range of values that a measure can take, based on statistical uncertainty and the fact that the data were derived from a sample of households across the country.

## Dataset information and descriptive statistics

```{r}
info_df <- final_df %>%
  summarize("Number of cities"
```
